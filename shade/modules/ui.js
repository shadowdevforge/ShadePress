...
// --- Theme Application Helper (Unchanged) ---
...
export function initUI(state) {
  ...
  // NEW: Render collapsible groups
  // We check if the group config has 'collapsed: true' to set initial state
  ...
  // 5. Sidebar Toggle Logic (Collapsible Groups)
  sideEl.addEventListener("click", (e) => {
    // Check if the click is on the header or inside the header
...
export function updateActiveLinks(hash, config) {
  ...
  links.forEach((link) => {
    ...
    if (isActive) {
      ...
      // If the active link is inside a collapsed group, open it.
